...
static request_t * new_request( bsc_step_t step ) 
{
    ...
    if ( delay >= s_bsc.horizon || 
        ...
        unsigned * new_n_requests = calloc( new_horizon, sizeof(unsigned));
        if ( ! new_n_requests ) 
        ...
        unsigned new_max_requests = s_bsc.max_requests_per_step; 
        ...
        request_t * new_queue = calloc( (size_t)
                new_horizon * new_max_requests, sizeof(request_t) );
        ...
        const void * src, void * dst, bsc_size_t offset, bsc_size_t size)
{
    ...
    request_t * req = new_request( depends );
    req->kind = COMM;
    req->comm = r;
    return depends + 1;
}
        ...
        if (s_bsc.horizon > 0) {
            for ( i = 0; i < s_bsc.n_requests[s_bsc.queue_start]; ++i ) {
                request_t * r = s_bsc.queue + 
                 ...
                 if ( r->kind == COMM ) {
                    if ( r->comm.src_pid == bsp_pid() ) {
                        bsp_put( r->comm.dst_pid, 
                                r->comm.src_addr, r->comm.dst_addr, 
                                r->comm.offset, r->comm.size );
                    }
                    else
                    {
                        bsp_get( r->comm.src_pid, r->comm.src_addr,
                                r->comm.offset, r->comm.dst_addr, 
                                r->comm.size );
                    }
                }
            }
        ...
        if (s_bsc.horizon > 0) {
            for ( i = 0; i < s_bsc.n_requests[s_bsc.queue_start]; ++i ) {
                request_t * r = s_bsc.queue + 
                    s_bsc.queue_start * s_bsc.max_requests_per_step + i ;
                if ( r->kind == EXEC ) {
                    bsc_reduce_t f = r->exec.func;
                    void * a = r->exec.accum;
                    const void * xs = r->exec.xs;
                    bsc_size_t size = r->exec.size;
                    (*f)( a, xs, size );
                }
            }
...
bsc_group_t bsc_group_create( unsigned color )
{
    ...
    if (!all_colors || !g)
        bsp_abort("bsc_split: Insufficient memory");
                      ...
                      const void * src, void * dst, bsc_size_t size )
{
    ...
    bsp_pid_t s = g?g->local_pid:bsp_pid();
    for ( i = 0; i < n; ++i ) {
        bsc_put( depends, g?g->members[i]:i, src + i * size, 
                dst, s * size, size );
    }
    return depends + 1;
}
...
{
    const group_t * g = group;
    (*reducer)( dst, src, size * nmemb );
    depends = bsc_gather( depends, root, group, dst, tmp_space, size );
    if ( root == bsp_pid() )
        bsc_exec_reduce( depends-1, reducer, dst, tmp_space, 
        ...
        bsc_reduce_t reducer, bsc_size_t nmemb, bsc_size_t size )
{
    ...
    return bsc_exec_reduce( depends-1, reducer, dst, tmp_space, 
                size * (g?g->size:bsp_nprocs())  );
}
